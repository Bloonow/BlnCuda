...
int main(int argc, char *argv[]) {
    ...
    matrix_init(&A, M, K, batchCount); matrix_init(&B, K, N, batchCount);
    matrix_init(&C0_ROW, M, N, batchCount, 0.f); matrix_init(&C0_COL, M, N, batchCount, 0.f);
    ...
    cudaFree(C0_ROW); cudaFree(C0_COL);
    cudaFree(C1); cudaFree(C2);
    return 0;
}