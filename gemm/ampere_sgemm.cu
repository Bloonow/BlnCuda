...
void ampere_sgemm_rrr_128x256x8_kernel(
    ...
    // 重用 256 * 32 * float 共享内存空间，行主序写回矩阵 C，每次写回一个分区的 16 * 32 * 8 Warp = 128 * 32 * float 数据
 ...
 */
__global__ __launch_bounds__(256)
void ampere_sgemm_rrr_not_frag_buffer_128x256x8_kernel(
    const float *A, const float *B, float *C, const float alpha,
    const uint32_t M, const uint32_t N, const uint32_t K
) {
    // A and B Threadblock Tile on shared memory (double buffer)
    ...
    // A, B Thread Tile on register, C Thread Tile on register (double buffer)
    float A_frag[16], B_frag[8], C_frag[16][8] = { 0 };
    ...
    for (uint32_t num_k_tiles = (K + 7) / 8 - 1; num_k_tiles > 0; --num_k_tiles) {
        #pragma unroll
        for (int k_frag = 0; k_frag < 8; ++k_frag) {
            // 读取计算所需的 A_frag 和 B_frag 并写入寄存器
            ptx::lds(A_frag[0], A_frag[1], A_frag[2], A_frag[3], A_lds_addr + k_frag * 132 * sizeof(float));
            ptx::lds(A_frag[4], A_frag[5], A_frag[6], A_frag[7], A_lds_addr + (k_frag * 132 + 16) * sizeof(float));
            ptx::lds(A_frag[8], A_frag[9], A_frag[10], A_frag[11], A_lds_addr + (k_frag * 132 + 32) * sizeof(float));
            ptx::lds(A_frag[12], A_frag[13], A_frag[14], A_frag[15], A_lds_addr + (k_frag * 132 + 48) * sizeof(float));
            ptx::lds(B_frag[0], B_frag[1], B_frag[2], B_frag[3], B_lds_addr + k_frag * 256 * sizeof(float));
            ptx::lds(B_frag[4], B_frag[5], B_frag[6], B_frag[7], B_lds_addr + (k_frag * 256 + 32) * sizeof(float));
            // K_tile 次计算即将执行完毕，将下一个 A_tile 和 B_tile 写入共享内存
            if (k_frag == 7) {
                ptx::ldg_sts_commit();
                __syncthreads();
